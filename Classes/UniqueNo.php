<?php
  ...
  class UniqueNo{
		 ...
		 */
		public function formNo($seed, $length, $ymd, $prefix){
			if($ymd){
		 ...
		 */
		public function isNoUsed(DataModel $Db, $no){
			$used = false;
			$Db->selectData(['unumber'], ['unumber'=>$no]);
			if($used = $Db->fetchRecords()) return $used;
		}
		 ...
		 * @return void
		 */
		public function storeNo(DataModel $Db, $no){
			$Db->insertData(['unumber'=>$no]);
			if($id = $Db->getLastInsertID()) return $id;
		}
		 ...
		 */
		public function fromDb(DataModel $Db, $length, $ymd=true, $prefix=""){
			$Db->setTable(TRANSACTION);
			$seed = 1;
